...
const API_URL = 'https://seven89tx-ls.onrender.com/taixiu/lichsu';
...
app.get('/789/lichsu', async (req, res) => {
  try {
    const data = await fetchData();
    if (data && data.data) {
      await verifyPredictions('b52', data.data);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`789 Prediction API running on port ${PORT}`);
  console.log('Available endpoints:');
  console.log('  GET /789 - Get prediction for next 789 round');
  console.log('  GET /789/lichsu - Get prediction history');
