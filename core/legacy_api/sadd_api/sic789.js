...
// Thiết lập việc lấy dữ liệu định kỳ (mỗi 3 giây)
clearInterval(fetch_interval);
fetch_interval = setInterval(fetch_and_process_history, 3000); 
console.log(`🔄 đang thiết lập fetch api mỗi 3 giây tại url: ${api_url}`);

// GET /api/sicbo/789 (ENDPOINT DỰ ĐOÁN CHÍNH)
app.get("/api/sicbo/789", async () => {
...
// GET /api/sicbo789/history (ENDPOINT LỊCH SỬ ĐÃ ĐƯỢC THAY ĐỔI)
app.get("/api/sicbo789/history", async () => { 
...
`;
    ...
    console.log(`   ➜ get /api/sicbo/789   → http://${public_ip}:${port}/api/sicbo/789`);
    console.log(`   ➜ get /api/sicbo789/history   → http://${public_ip}:${port}/api/sicbo789/history`);
};
